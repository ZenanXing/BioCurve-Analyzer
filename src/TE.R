...
df_et_exp <- reactive({
  ...
  colnames(df_temp)[(n_var+1):ncol(df_temp)] <- c("Model", "Maximum_Response", "Minimum_Response",  "Response at T\u2085\u2080", "T\u2085\u2080_Mean", "T\u2085\u2080_SE", "T\u2085\u2080 SD", "T\u2085\u2080_LowerBound", "T\u2085\u2080_UpperBound")
  return(df_temp)
})
...
T50_table <- reactive({
  ...
  colnames(df_temp)[(n_var+1):ncol(df_temp)] <- c("Model", "Response at T\u2085\u2080", "T\u2085\u2080 Mean", "T\u2085\u2080 SE", "T\u2085\u2080 LowerBound", "T\u2085\u2080 UpperBound")
  return(df_temp)
})
...
L_P_te <- reactive({
  ...
  if (n_var == 0) {
    ...
    anno_df <- isolate({df_et()}) %>% dplyr::select((n_var+7):(n_var+14))
    ...
    # annotation dataframe
    anno_df <- isolate({df_et()}) %>% dplyr::select(1:n_var, (n_var+7):(n_var+14))
    anno_df[, (n_var+1):ncol(anno_df)] <- lapply(anno_df[, (n_var+1):ncol(anno_df)], as.numeric)
    # plot
